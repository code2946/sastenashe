...
export const OptimizedImage = memo(function OptimizedImage({ 
  ...
  const loadImage = useCallback(async () => {
    ...
    try {
      ...
      // For TMDB images (direct or proxy), load with smart fallback and timeout
      if (src.includes('/api/tmdb-image') || src.includes('image.tmdb.org')) {
        const img = new Image()
        
        // Add timeout for slow loading images (3 seconds)
        const timeout = setTimeout(() => {
          console.warn('Image loading timeout, using placeholder:', src)
        ...
        const handleSuccess = () => {
          ...
          setImageSrc(src)
          setIsLoading(false)
          ultraFastImageLoader.preloadImages([src])
          onLoad?.()
        }
        ...
        const handleError = () => {
          ...
          // If it's a direct TMDB URL that failed, try our proxy as fallback
          if (src.includes('image.tmdb.org')) {
            const pathMatch = src.match(/\/t\/p\/[^/]+(.+)$/)
            if (pathMatch) {
              const imagePath = pathMatch[0]
              const proxyUrl = `/api/tmdb-image?path=${encodeURIComponent(imagePath)}`
